...
`APP_REDIS_ENABLED=false`로 설정하면 공통 RedisTemplate 설정과 Actuator Redis health를 함께 끈다. Redis 서버가 없는 테스트/임시 환경에서 전체 health가 Redis 때문에 `DOWN`으로 보이지 않게 하기 위함이다.
...
`APP_REDIS_ENABLED=true`이면 Actuator health에 Redis 상태가 포함된다.
